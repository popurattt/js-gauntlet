...
searchButton.addEventListener('click', () => {
    ...
    const foundGuest = guests.find(guest => {
        ...
        return name.toLowerCase() === searchValue;
    });

    if (foundGuest) {
        const [name, tag] = foundGuest.split(":");
        presentation.textContent = `L'invité ${name} a été trouvé. Son tag est ${tag}.`;
        admitted.push(name);
    } else {
        presentation.textContent = `L'invité ${searchInput.value.trim()} n'a pas été trouvé.`;
        refused.push(searchInput.value.trim());
    }
...
function updateList() {
    admittedPara.textContent = `Admit: ${admitted.join(', ')}`;
    refusedPara.textContent = `Refuse: ${refused.join(', ')}`;
}